...
def Input():
    data_dir = pjoin(dirname(sio.__file__),
                     'C:/Users/Xuyang/Desktop/Spring-2021-PhD-Xuyang/Research/Fault-Detection-Self-Driving-Cars/Datasets/CARAL-Simulator/Town-03/MS/')
    csv_fname = pjoin(data_dir, 'CarlaTown03-30Vehicles-ML-fault-MS-2.csv')
    # sample = pd.read_csv(csv_fname,header=None)
    sample = pd.read_csv(csv_fname)
    attributes = sample.columns.values[:-1]
    [N,L] = np.shape(sample)
    # Extract the label of the data from the data frame
    data, label = sample.iloc[:108000,0:2].values, sample.iloc[:108000,L-1].values
    ...
    for i in range(np.shape(P)[0]):
        ...
        if not np.any(np.isin(merge_nei, Neighbor_Index)):
            ...
            if Neighbor_Index < (histnonum):
                ...
                if boundary_fitness < 0.85 * min(current_fit, neighbor_fit):
                    merge_n = False
                else:
                    merge_n = True
                if merge_n:
...
#    scaler = MinMaxScaler()
#    scaler = scaler.fit(combined_reps)
#    combined_reps = scaler.transform(combined_reps)
#    combined_reps[:,:2] /= 360 
#    combined_reps[:,6]/360
    ...
    for i in range(len(com_falset)):
        ...
        curr_fel = curr_fel / (np.sum(curr_fel))
...
def time_sequence(data, step):
    c = 0
    new_data = []
    for i in range(step,np.shape(data)[0]):
        new_data.append(np.reshape(np.matrix.flatten(data[c:i,:]), (1,len(np.matrix.flatten(data[c:i,:])))))
    new_data = np.asarray(new_data)
    new_data = np.reshape(new_data, (np.shape(new_data)[0], np.shape(new_data)[2]))
    return new_data
...
if __name__ == '__main__':
    ...
    for t in range(T):
        ...
        num_faults = np.shape(novelfal)[0]+np.shape(mergedfa)[0]
#        if t == 4:
#            print('Error Check!!!')
...
#            print('Fault happens!!!')
        ...
        predlabelset = np.unique(predlabel)
...
#            predict_fault = False
        if len(predlabelset) == 1:
            if predlabelset[0] != 'Normal ' and len(np.where(predlabel!='Normal ')[0])>=threshold:
                predict_fault = True
            else:
                predict_fault = False
        else:
            for tl in predlabelset:
                if tl != 'Normal ' and len(np.where(predlabel!='Normal ')[0])>=threshold:
                    predict_fault = True
                    break
                else:
                    predict_fault = False
        if predict_fault and true_fault:
            ...
            faultcount += 1
            subcluster_representatives, subcluster_labelrep = Extract_Labels(normalsub_info, faultsub_info, dim)
#            consider_sample = subcluster_representatives
#            consider_label = subcluster_labelrep
...
#        fea_relinfo[key] = attributes[fea_relinfo[key]]
    ...
    if normalcounts > 0:
        ...
        PPV = faultcount / (falsecount + faultcount) * 100
...
#    print("Associated sensors with fault classes: ", fea_relinfo)
    end = time.time()
...
#    plt.savefig('results_town03_MS.png')
    plt.show()
    ...
    plt.legend()
    plt.yticks([0, 1, 2, 3, 4, 5])
#    plt.savefig('results_town03_MS_miss.png')
    plt.show()
#    plt.figure(3)
#    cycol = cycle('bgcmrk')
...
#    lines = []
#    labels = []
#    colors = get_cmap(len(fault_types))
#    for i in range(2):
#        for j in range(2):
#            idx = i*2+j
#            key = fault_types[idx]
#            if idx < 3:
#                s_idx = np.argsort(fea_relinfo[key])[-2:]
#                s_lw = np.asarray([1.5,1.5,1.5])
#                s_ec = np.asarray(['yellow','yellow','yellow'])
#                s_ec[s_idx] = 'black'
#                s_lw[s_idx] = 3.0
#            else:
#                s_idx = np.argsort(fea_relinfo[key])[-3:]
#                s_lw = np.asarray([1.5,1.5,1.5])
#                s_ec = np.asarray(['yellow','yellow','yellow'])
#                s_ec[s_idx] = 'black'
#                s_lw[s_idx] = 3.0
#            axes[i,j].barh(np.arange(len(sensors)),fea_relinfo[key],label=str(key),color=next(cycol),edgecolor=s_ec,linewidth=s_lw)
#            line, label = axes[i,j].get_legend_handles_labels()
#            lines += line
#            labels += label
#    plt.legend(lines, labels, loc = 'upper right',bbox_to_anchor=(1.51, 2.25))
...
##    plt.savefig('sensor_town03_MS.png')
#    plt.show()
